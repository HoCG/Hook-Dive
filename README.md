...
useImperativeHandle은 ref를 사용할때 적용가능한 훅이다. 자식에서 부모의 함수를 사용할때 prop을 쓰지않는가? 근데 부모에서 자식의 함수를 쓰고싶다고 치자. 어떻게 사용이 가능할까? 바로 이 useImperativeHandle훅을 사용하면 된다. ref를 넘겨받은 자식은 해당 ref에 대하여 부모에서 사용이 가능한 함수들을 정의해둘 수 있다. 그럼 ref.current를 통해 해당 함수에 접근이 가능하다! 이러면 좋은게 부모에서 자식의 DOM을 수정해야하는 상황이 있을때 useImperativeHandle훅으로 자식에서 DOM수정을 하도록 할 수 있다.

이렇게만 보면 useImperativeHandle훅이 굉장히 좋아보일텐데 한가지 큰 문제점이 있다. 바로 리액트의 컨셉에서 벗어난 코딩을 해야한다는 것이다. 이게 무슨소릴까? 바로 명령형으로 코드를 작성해야 한다는게 문제라는거다. 기본적으로 리액트는 state를 통해 ui를 변경시키는 선언형 프로그래밍을 지향한다. useImperativeHandle의 경우 이런 방식이 아니기 때문에 특별한 케이스가 아니면 사용을 안하는게 좋다.
