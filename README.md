...
useImperativeHandle은 forwardRef를 사용시 적용가능한 훅이다. 자식에서 부모의 함수를 사용할때 prop을 쓰지않는가? 근데 부모에서 자식의 함수를 쓰고싶다고 치자. 어떻게 사용이 가능할까? 물론 부모에서 함수를 작성한 후에 자식으로 넘겨주는 방법도 있겠지만 엄연히 자식에서 동작해야하는 함수가 부모에 있다는게 너무 이상할 뿐더러 부모 컴포넌트 코드의 길이가 이상하게 긿어질 수 있다. 이에 우리는 useImperativeHandle훅을 통해 자식의 함수에 접근이 가능해진다. ref를 넘겨받은 자식에서 useImperativeHandle훅에 함수를 정의해두면 ref.current를 통해 해당 함수에 접근이 가능하다! 
