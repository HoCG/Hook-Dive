...
이건 결과가 어찌될까? 또 false다. 왜 그럴까? 자바스크립트는 A = {}; B = A; A === B; 이런식으로 비교하는게 아니라면 객체는 겉보기에는 값을 저장하는것처럼 보이지만 사실은 메모리 주소를 저장하기 때문이다. 따라서 darkMode의 상태가 변할때마다 person이 새롭게 정의되고 person이라는 값이 아무리 기존값과 동일하다지만 메모리 위치를 비교하기 때문에 person === person은 false를 뱉게 되는거다. 이때 useMemo를 사용하는 것이다.
...
나만의 커스텀 훅을 만들어봤다. 용도를 설명해보자면 useQuery와 useMutation을 활용해서 todo정보를 가져오거나 보낼 수 있는 훅이였다. 이렇게 만들어보니 막상 별거아니라 놀랐다. 공통되는 사항이 많은 훅이 있다면 이렇게 커스텀 훅을 통해서 관리하는 방법을 채택해봐야겠다는 생각이 들었다.

# Hook-Dive 후기

드디어 훅다이브가 끝났다...👏👏👏 리액트내에 있는 다양한 훅들, 라이브러리로 제공되고있는 훅들을 다뤄봤는데 개인적으로 너무 재밌었다. 리액트와 깊게 친해지는 시간을 가지기도 했고 개념이해 >> 개념복습차원에서 문서를 보지않고 글 작성 >> 실습해보기 의 나만의 루틴도 구축한 시간이였다. 그리고 이번 hook-dive를 통해 제일 신선한 충격을 받았던건 useRef와 useQuery의 활용도... 내가 useRef를 진짜 원숭이처럼(?) 쓰고있었구나 생각이 들었고 useQuery를 제대로 못쓰고 있었구나 하며 깨닫는 시간이였다. 이래서 개념공부가 중요한가보다... 앞으로는 무작정 프로젝트에 적용시킬 생각보단, 공부를 철저히 하고 프로젝트에 적용해야겠다라고 깨닫기도 했다. 아무튼 재밌었다 훅다이브!
